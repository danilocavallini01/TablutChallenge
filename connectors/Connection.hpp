...
namespace Connectors
{
    ...
    public:
        Connection(std::string __name, std::string __ipAddress = "127.0.0.1", bool __isWhite = true) : _name(__name), _ipAddress(__ipAddress), _isWhite(__isWhite)
        {
            struct hostent *host;
            ...
            if (_sd < 0)
            {
                std::cerr << "SOCKET INITIALIZE ERROR (BAD DESCRIPTOR)" << std::endl;
                exit(1);
            }
        ...
        */
        void send(std::string &__msg)
        {
            ...
            if (n < 0)
            {
                std::cerr << "ERROR WHILE WRITING SIZE OF MESSAGE TO SOCKET" << std::endl;
            }
            ...
            if (n < 0)
            {
                std::cerr << "ERROR WHILE WRITING MESSAGE TO SOCKET" << std::endl;
            }
        }
        ...
        */
        std::string receiveString()
        {
            ...
            {
                std::cerr << "ERROR WHILE READING LENGTH OF MESSAGE" << std::endl;
                exit(1);
            }
            strlen = ntohl(strlen);
